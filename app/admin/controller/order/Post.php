    ...
    public  function  order(){
        if (Request::isPost()) {
            ...
            $list = Db::name('post_order')
                ->alias('postOrder')
                ->join('order','order.id=postOrder.orderId','left')
                ->join('post','post.id=postOrder.post_id','left')

//                ->join('admin' ,'admin.id=account.adminId')
                //  ->where('nickname', 'like', '%' . $keys . '%')
                ->field('postOrder.*,post.name postName')
                ->paginate(['list_rows' => $this->pageSize, 'page' => $page])
                ->toArray();
    ...
    public function stateOrder()
    {
        ...
        if ($id) {
            ...
            $link = PostOrderModel::find($id);
            $where['status'] = $link['status'] ? 0 : 1;
            PostOrderModel::update($where);
